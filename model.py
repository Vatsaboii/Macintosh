...
from file_utils import random_hash  # Ensure file_utils.py is in the same directory

logging.basicConfig(level=logging.INFO)

def load_detection_model(model_file, config_file):
    """Load pre-trained face detection model."""
    logging.info(f"Loading face detection model from {model_file} and {config_file}...")
...
def encode_faces(recognition_model, shape_predictor, image, faces):
    ...
    for (startX, startY, endX, endY) in faces:
        ...
        face_encodings = recognition_model.compute_face_descriptor(image, full_object_detections)
        if face_encodings:
            encodings.append(np.array(face_encodings[0]))
        else:
            logging.warning(f"Failed to encode face at location: ({startX}, {startY}, {endX}, {endY})")
    return encodings
...
def draw_boxes(image, boxes, names, color=(0, 255, 0)):
    ...
    for ((startX, startY, endX, endY), name) in zip(boxes, names):
        ...
        cv2.putText(image, name if name else "Unknown", (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

def main(input_image_path, base_folder):
    model_file = "C:/Users/Chakri/OneDrive/Desktop/hackthon/res10_300x300_ssd_iter_140000.caffemodel"
    config_file = "C:/Users/Chakri/OneDrive/Desktop/hackthon/deploy.prototxt.txt"
    recognition_model_file = "C:/Users/Chakri/OneDrive/Desktop/hackthon/dlib_face_recognition_resnet_model_v1.dat"
    shape_predictor_file = "C:/Users/Chakri/OneDrive/Desktop/hackthon/shape_predictor_68_face_landmarks.dat"
    ...
    try:
            ...
            if os.path.isdir(person_folder):
                ...
                for file_name in os.listdir(person_folder):
                    if file_name.endswith(".jpg") or file_name.endswith(".jpeg") or file_name.endswith('.png'): 
                        face_image_path = os.path.join(person_folder, file_name)
                        face_image = cv2.imread(face_image_path)
                        if face_image is not None:
                            face_encoding = encode_faces(recognition_model, shape_predictor, face_image, [(0, 0, face_image.shape[1], face_image.shape[0])])
        ...
        face_encodings = encode_faces(recognition_model, shape_predictor, image, faces)
        recognized_names = recognize_faces(face_encodings, known_encodings, known_names)
...
if __name__ == "__main__":
    tags = main(input_image_path='C:/Users/Chakri/OneDrive/Desktop/hackthon/WhatsApp Image 2024-06-08 at 14.37.00_7729bf4d.jpg', base_folder='C:/Users/Chakri/OneDrive/Desktop/hackthon/static/photos/jaswanth')
    if tags:
